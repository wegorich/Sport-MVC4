...
namespace SportSecond.BootstrapSupport
{
    public static class DefaultScaffoldingExtensions
    {
        ...
        {
            Type elementType = ObjectContext.GetObjectType(model.GetType()).GetElementType() ??
                               ObjectContext.GetObjectType(model.GetType()).GetGenericArguments()[0];
            return
        ...
        {
            return
                ObjectContext.GetObjectType(model.GetType())
                             .GetProperties()
                             .OrderBy(x => x.MetadataToken)
    ...
    public static class PropertyInfoExtensions
    {
        ...
        {
            return (model == null)
                       ? LabelFromType(new Object().GetType())
                       : LabelFromType(ObjectContext.GetObjectType(model.GetType()));
        }

        public static string GetLabel(this IEnumerable model)
        {
            Type elementType = model.GetType().GetElementType() ?? model.GetType().GetGenericArguments()[0];
